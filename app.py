...
while True:
    ...
    df_pontos_saida = df_pontos_saida[df_pontos_saida['equ_id_equipamento'].isin(equipamentos_saida)]
    ...
    for placa in placas_distintas.itertuples(index=False):
        ...
        df_passou = df_pontos_saida[(df_pontos_saida['placa'] == placa[0]) & (df_pontos_saida['pas_dh_passagem'] >= limite_dias)]
        # Groupby por placa e maior pas_dh_passagem e com o ponto da passagem com maior pas_dh_passagem

        #se tiver registro, retorna verdadeiro
        if len(df_passou) > 0:
            send_message("Placa: " + placa[0] + " passou no Equipamento: " + str(placa[2]) + " na Data: " + str(placa[1]))
            send_message(" e também passou no Equipamento: " + str(df_passou['equ_id_equipamento']) + " na Data: " + str(df_passou['pas_dh_passagem']) )
